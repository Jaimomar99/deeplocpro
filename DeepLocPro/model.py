...
class EnsembleModel(nn.Module):
	...
	def __init__(self):
		...
		for i in range(5):
			for j in range(5):
				...
				model_file = importlib.resources.files(__name__).joinpath(f"models/checkpoints/model_{i}_{j}")
				with importlib.resources.as_file(model_file) as path:
	...
	def forward(self, embeddings, masks):
		...
		self.subcel_clfs = self.subcel_clfs.to(device)
	...
	def embed_batch(self, sequences: List[str], repr_layers=[33]):
		...
		for seq in sequences:
			...
			self.esm_model = self.esm_model.to(device=device)
